...
async def parse_channel(client: TelegramClient, channel_name: str, limit: int = 10) -> int:
    ...
    try:
        ...
        logger.exception(f"⚠️ Неожиданная ошибка при получении сущности '{channel_name}': {e}")
        return 0
    ...
    try:
        async for message in client.iter_messages(entity, limit=limit):
            ...
            try:
                ...
                logger.error(f"❌ Ошибка сохранения поста {message.id}: {e}", exc_info=True)
        ...
        logger.info(f"📥 Канал '{channel_name}' обработан. Сохранено постов: {saved_count}")
        return saved_count

    except Exception as e:
        logger.error(f"🔥 Критическая ошибка при парсинге канала '{channel_name}': {e}", exc_info=True)
        return 0


async def parse_all_active_channels(client: TelegramClient, limit_per_channel: int = 10) -> int:
    """
...
async def start_scheduled_parsing(client: TelegramClient, interval: int = 3600, limit_per_channel: int = 10):
    """
    ...
    while True:
        try:
            ...
            logger.critical(f"🔥 Критическая ошибка в планировщике: {repr(e)}", exc_info=True)
